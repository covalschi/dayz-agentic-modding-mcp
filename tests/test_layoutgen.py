...
def page(body, root=None, layout="my_page", note=""):
...
def build(desc):
    return build_layout(desc, tokens(), "ui/MyMod/my_page.json", "MyMod/gui/layouts")
...
def test_a_frame_root_with_one_label_renders_the_vanilla_shape():
    ...
    assert list(out.files) == ["MyMod/gui/layouts/my_page.layout"]
    text = out.files["MyMod/gui/layouts/my_page.layout"]
    assert text.splitlines()[0] == "// GENERATED by dayz-mcp layout_build from ui/MyMod/my_page.json -- edit the source, not this file."
...
def test_at_on_a_page_child_is_noted_but_hidden_overlays_are_not():
    ...
    assert out.notes == ["ui/MyMod/my_page.json root.0: `at` on a page child -- build the page with vbox/hbox"]
...
def test_a_root_can_anchor_and_offset_itself():
    ...
    assert "FrameWidgetClass Win {\n visible 1\n position 40 30\n size 640 518\n" in out.files["MyMod/gui/layouts/my_page.layout"]


def test_a_chip_says_it_ignores_h():
    out = build(page({"frame": {"name": "Row", "size": [300, 40], "children": [
        {"chip": {"name": "Chip", "h": 10}}]}}))
    assert out.notes == ["ui/MyMod/my_page.json root.0.0: chip ignores h -- it is its parent's full height"]
...
def test_a_click_rows_color_is_noted_as_unpainted():
    ...
    assert out.notes == ["ui/MyMod/my_page.json rows.r: row color is only painted on a plain (non-click, non-stack) row"]
...
def test_literal_colors_and_fonts_are_allowed_but_noted():
    ...
    assert out.notes == [
        "ui/MyMod/my_page.json root.0: color given as a literal -- use a $color token",
        "ui/MyMod/my_page.json root.0: font given as a literal -- use a $font token",
    ]
...
def test_a_non_root_button_says_it_ignores_children():
    ...
    assert out.notes == ["ui/MyMod/my_page.json root.0: button ignores children outside the root"]
    assert clean(out.files["MyMod/gui/layouts/my_page.layout"]) == []
...
def test_field_puts_the_edit_box_inside_a_frame_and_a_fill():
    ...
    assert "MultilineEditBoxWidgetClass Body {" in out.files["MyMod/gui/layouts/my_page.layout"]
    assert "     lines 8\n" in out.files["MyMod/gui/layouts/my_page.layout"]
...
def test_text_wraps_and_sizes_its_height_to_the_text():
    ...
    assert "MultilineTextWidgetClass Plain {" in out.files["MyMod/gui/layouts/my_page.layout"]
...
def test_list_writes_scroll_spacer_and_a_row_file_of_the_scroll_width_minus_the_bar():
    ...
    page_text = out.files["MyMod/gui/layouts/my_page.layout"]
    assert (
    ...
    assert row.splitlines()[0] == "// GENERATED by dayz-mcp layout_build from ui/MyMod/my_page.json (rows.contact_row) -- edit the source, not this file."
...
PAGE = {"layout": "my_page", "root": {"frame": {"name": "MyPage", "size": [640, 518], "inset": 20}},
...
def project(tmp_path: Path) -> Path:
    ...
    (tmp_path / "ui" / "MyMod" / "my_page.json").write_text(json.dumps(PAGE), encoding="utf-8")
...
def test_build_project_writes_changed_files_only(tmp_path):
    ...
    assert first.written == ["MyMod/gui/layouts/my_page.layout"] and first.unchanged == []
    out = root / "MyMod" / "gui" / "layouts" / "my_page.layout"
    assert out.read_text(encoding="utf-8").startswith("// GENERATED by dayz-mcp layout_build from ui/MyMod/my_page.json")
    assert b"\r\n" not in out.read_bytes()
    second = build_project(root, ["MyMod"], {}, write=True)
    assert second.written == [] and second.unchanged == ["MyMod/gui/layouts/my_page.layout"]
    assert second.sources == {"MyMod/gui/layouts/my_page.layout": "ui/MyMod/my_page.json"}


def test_build_project_with_write_false_reports_but_touches_nothing(tmp_path):
    root = project(tmp_path)
    report = build_project(root, ["MyMod"], {}, write=False)
    assert report.written == ["MyMod/gui/layouts/my_page.layout"]
...
def test_build_project_refuses_two_descriptions_for_one_file(tmp_path):
    ...
    with pytest.raises(LayoutGenError, match="my_page.layout is generated by two descriptions"):
        build_project(root, ["MyMod"], {}, write=True)
...
def test_main_refuses_an_unknown_mod_and_builds_a_known_one(tmp_path, capsys):
    ...
    assert "wrote MyMod/gui/layouts/my_page.layout" in capsys.readouterr().out
...
def test_a_screen_root_is_proportional_and_holds_anchored_exact_children():
    out = build_layout({"layout": "my_menu", "root": {"frame": {"name": "Root", "size": "screen", "priority": 2, "children": [
        {"panel": {"name": "Backdrop", "size": "fill", "color": "$none", "priority": 1}},
        {"frame": {"name": "Device", "anchor": "center", "size": [1403, 590], "priority": 2}}]}}},
        tokens(), "ui/MyMod/my_menu.json", "MyMod/gui/layouts")
    text = out.files["MyMod/gui/layouts/my_menu.layout"]
...
def test_a_button_root_is_bare():
    out = build_layout({"layout": "my_tab", "root": {"button": {"name": "MyTab", "size": [60, 60], "children": [
        {"panel": {"name": "TabActive", "size": "fill", "color": "$pick", "hidden": True}}]}}},
        tokens(), "ui/MyMod/my_tab.json", "MyMod/gui/layouts")
    text = out.files["MyMod/gui/layouts/my_tab.layout"]
...
def test_a_button_root_notes_the_attributes_it_ignores():
    ...
    out = build_layout({"layout": "my_tab", "root": {"button": {
        "name": "MyTab", "size": [60, 60], "text": "#STR_HI", "font": "small",
        "bg": "$panel", "edge": "$accent", "glyph": True}}},
        tokens(), "ui/MyMod/my_tab.json", "MyMod/gui/layouts")
    text = out.files["MyMod/gui/layouts/my_tab.layout"]
    assert ' text ""\n' in text and "MyTabEdge" not in text and "MyTabText" not in text
    assert out.notes == ["ui/MyMod/my_tab.json root: button root ignores text/font/bg/edge/glyph -- give it children"]
    assert clean(text) == []
...
def test_bar_is_a_track_with_an_empty_fill():
    out = build({"layout": "my_page", "root": {"frame": {"name": "P", "size": [300, 100], "children": [
...
def test_map_is_a_clipped_map_widget_without_children():
    out = build({"layout": "my_page", "root": {"frame": {"name": "P", "size": [1306, 518], "children": [
    ...
    with pytest.raises(LayoutGenError, match="paints over its children"):
        build({"layout": "my_page", "root": {"frame": {"name": "P", "size": [10, 10], "children": [
...
def test_bar_refuses_a_plain_color():
    ...
    with pytest.raises(LayoutGenError, match=r"bar does not take \['color'\]"):
        build({"layout": "my_page", "root": {"frame": {"name": "P", "size": [300, 100], "children": [
